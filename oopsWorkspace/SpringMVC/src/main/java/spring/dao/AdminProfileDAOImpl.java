...
public class AdminProfileDAOImpl implements AdminProfileDAO {
	...
	@Override
	public void removePet(int userId) {
	...
	@Override
	public void removeContactInfo(int userId) {

		Session session = this.sessionFactory.getCurrentSession();
		int id = (Integer)(session.createQuery("select id from ContactInfo where userId="+String.valueOf(userId)).uniqueResult());
		...
		for(int i:userIdList)
		{
			...
			if(d.after(u.getExpiresOnDate()))
			{
				if(u.getLoginTime().before(u.getExpiryEmailSentDate()))
				{
					...
					removeContactInfo(i);
					removePet(i);
					//session.createQuery("delete from Pet where userId="+String.valueOf(i));
					
				  // session.createQuery("delete from ContactInfo where userId="+String.valueOf(i));
